...
--     DECLARE
--         latest_active_transaction RECORD;
--         -- jakarta_timestamp TIMESTAMPTZ;  -- Declare variable to hold the converted timestamp
--     BEGIN
--         -- Convert the NEW.timestamp_pembayaran to 'Asia/Jakarta' timezone
--         -- jakarta_timestamp := (NEW.timestamp_pembayaran AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta';

--         -- Select the most recent transaction that is still active
--         SELECT * INTO latest_active_transaction
--         FROM Transaction
--         WHERE username = NEW.username AND end_date_time >= CURRENT_DATE
--         ORDER BY end_date_time DESC
...
--                 dukungan_perangkat = NEW.dukungan_perangkat,
--                 metode_pembayaran = NEW.metode_pembayaran,
--                 timestamp_pembayaran = jakarta_timestamp  -- Use the converted timestamp
--             WHERE username = NEW.username AND transaction_id = latest_active_transaction.transaction_id;
        
--         ELSE
--             -- Insert a new transaction record with the converted timestamp
--             INSERT INTO Transaction(username, nama_paket, start_date_time, end_date_time, metode_pembayaran, timestamp_pembayaran)
--             VALUES (NEW.username, NEW.nama_paket, NEW.start_date_time, NEW.end_date_time, NEW.metode_pembayaran, jakarta_timestamp);
--         END IF;
